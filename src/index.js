	...
	*/
    ...
    if (zrxOrder.takerFee.toString() !== '0') {
        ...
        if (
            ...
            console.log(
                'takerFeeAssetData not recognized: ' +
        ...
        console.table([
            {
                ...
                Input: displayTokens(inputAssetAmount, assetOrder[0]).padEnd(
                    22,
                    ' '
                ),
                Output: displayTokens(outputAssetAmount, assetOrder[0]).padEnd(
                    22,
                    ' '
                ),
                Profit: displayTokens(
                    netProfit.toString(),
                    assetOrder[0]
                ).padEnd(22, ' '),
                Timestamp: now(),
...
async function trade(
    ...
    /* 
从DyDx 交易所获得闪电贷 DAI
使用通过闪贷借入的 DAI 从 0x 购买 WETH
使用 1inch 找到最好的交易所出售在步骤 2 中获得的 WETH
偿还闪贷 DAI，剩余部分留作利润
  */
...
async function checkOrderBook(baseAssetSymbol, quoteAssetSymbol) {
    ...
    const quoteAssetAddress = ASSET_ADDRESSES[quoteAssetSymbol].substring(
        2,
        42
    );
...
//checkMarkets -> checkOrderBook -> checkArb -> trade
